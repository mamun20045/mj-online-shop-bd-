    ...
    <footer className="bg-gray-900 text-white pt-12 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div>
            <h3 className="text-xl font-bold mb-4 text-orange-500">{settings?.siteName || 'MJ Online Shop BD'}</h3>
            ...
            <div className="flex space-x-4 mt-6">
              ...
              {settings?.youtubeUrl && (
                <a href={settings.youtubeUrl} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              ...
              <li className="flex items-center">
                ...
                <span>{settings?.contactPhone || '+880181058092'}</span>
              </li>
              <li className="flex items-center">
                <Mail className="h-5 w-5 mr-2 text-orange-500 flex-shrink-0" />
                <span>{settings?.contactEmail || 'info@mjonlineshopbd.com'}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="border-t border-gray-800 mt-12 pt-8 text-center text-sm text-gray-500">
          <p>&copy; {new Date().getFullYear()} {settings?.siteName || 'MJ Online Shop BD'}. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
